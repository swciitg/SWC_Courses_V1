  ...
  User.findById(req.user._id)
    ...
    .exec(function (err, foundUser) {
      if (err) {
        ...
        console.log(foundUser.enrolled_courses);
  ...
  User.register(newUser, req.body.password, function (err, user) {
    ...
    token.save(function (err) {
      if (err) {
        ...
        req.flash(
          "verfError",
          "otp generation failed, please login and try again"
        );
    ...
    var mailOptions = {
      ...
      to: user.username,
      subject: "Account Verification Token from testotp",
      text:
        "Hello,\n\n" +
    ...
    transporter.sendMail(mailOptions, function (err) {
      if (err) {
        req.flash(
          "verfError",
          "verification email not send, if the error is persistance please contact SWC"
        );
        return res.redirect("/");
      }
      req.flash(
        "success",
        "Verification email sent, please check your inbox as well as junk mails"
      );
      res.redirect("/");
    });
  });
  ...
  Token.findOne({ token: req.params.id }, function (err, token) {
    ...
    User.findOne({ _id: token._userId }, function (err, user) {
      ...
      if (user.isverfied) {
        ...
        req.flash(
          "success",
          "User verified succesfully, please login and enjoy the courses"
        );
        res.redirect("/");
      }
...
router.get("/register/resetToken", function (req, res) {
  ...
  transporter.sendMail(mailOptions, function (err) {
    if (err) {
      req.flash(
        "verfError",
        "Verification email not sent please try again. If the issue persists, please contact SWC"
      );
      req.session.destroy(function (err) {
        req.logOut();
        return res.redirect("/");
    ...
    req.flash(
      "success",
      "An email has been sent to verify email address, please check your inbox and junk mails"
    );
    res.redirect("/");
  });
});
...
router.post("/login/forgot", function (req, res) {
  User.findOne({ username: req.body.username }, function (err, user) {
    ...
    var mailOptions = {
      from: process.env.GmailUser,
      to: user.username,
      subject: "Password reset request for your account on happylearning",
      text:
        "Hello,\n\n" +
        "Please reset your account password by clicking the link: \nhttp://" +
        req.headers.host +
        "/login/forgot/confirmation/" +
    ...
    transporter.sendMail(mailOptions, function (err) {
      ...
      req.flash(
        "success",
        "A password reset link has been sent to your email account. Please check the junk folder as well."
      );
  ...
  User.findOne(
    ...
    function (err, user) {
      ...
      var mailOptions = {
        from: process.env.GmailUser,
        to: user.username,
        subject: "Password reset for your account on happylearning",
        text:
          "Hello,\n\n" +
          "password for your account on happylearing has been changed on" +
          Date.now() +

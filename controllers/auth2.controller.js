...
exports.login = async (req, res) => {
    try {
        ...
        const { isAdmin, isverified, enrolled_courses_id, _id, name, enrolled_courses } = user;
        const data = {
            isAdmin,
            isverified,
            enrolled_courses_id,
        ...
        const token = await jwt.sign({ user: data }, config.get('jwtSecret'), { expiresIn: 3 * 3600 });
...
exports.register = async (req, res) => {
    try {
        ...
        if (password.length < 8) {
            return res.status(400).json({ msg: 'Password must have a minimum length of 8.' });
        }
